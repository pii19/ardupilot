...
#include "OGR_SensorTempMotor_Backend.h"
...
*/
OGR_SensorTempMotor_Backend::OGR_SensorTempMotor_Backend(OGR_SensorTempMotor::OGR_SensorTempMotor_State &_state) :
        state(_state)
{
...
void OGR_SensorTempMotor_Backend::update_status()
{
/*
    // check temperature
    if (state.temperature > state.max_temperature) {
        set_status(OGR_SensorTempMotor::OGR_SensorTempMotor_OutOfRangeHigh);
    } else if ((int16_t)state.temperature < state.min_temperature) {
        set_status(OGR_SensorTempMotor::OGR_SensorTempMotor_OutOfRangeLow);
    } else {
        set_status(OGR_SensorTempMotor::OGR_SensorTempMotor_Good);
    }
*/
}

// set status and update valid count
void OGR_SensorTempMotor_Backend::set_status(OGR_SensorTempMotor::OGR_SensorTempMotor_Status _status)
{
    state.status = _status;

    // update valid count
    if (_status == OGR_SensorTempMotor::OGR_SensorTempMotor_Good) {
 ...
 */
void OGR_SensorTempMotor_Backend::update_pre_arm_check()
{/*
    // return immediately if already passed or no sensor data
    if (state.pre_arm_check || state.status == OGR_SensorTempMotor::OGR_SensorTempMotor_NotConnected || state.status == OGR_SensorTempMotor::OGR_SensorTempMotor_NoData) {
        return;
    }
